...
class EKFSLAM:
    ...
    def Fx(self, x: np.ndarray, u: np.ndarray) -> np.ndarray:
        ...
        Fx[0, 2] = -u[0]*np.sin(x[2]) - u[1]*np.cos(x[2]) #legge til wraptopi?
    ...
    def h(self, eta: np.ndarray) -> np.ndarray:
        ...
        delta_m = m - x[:2] - rotmat2d(x[2]) @ self.sensor_offset
    ...
    def add_landmarks(
        ...
        Padded = la.block_diag([P, Gx @ P[:3, :3] @ Gx.T + Gz @ Rall.T])
    ...
    def associate(
        ...
        if self.do_asso:
            ...
            # q: Should one do something here?
            # a: No
            pass
    ...
    def update(
        ...
        if numLmk > 0:
            ...
            # TODO,
            S = H @ P @ H.T + np.kron(self.R, numLmk)
            assert (
            ...
            if za.shape[0] == 0:
                ...
                W = P @ Ha.T @ la.cho_solve(Sa, np.ones(len(Sa)))
                # TODO, Kalman update
                etaupd = eta + W @ v
        ...
        if self.do_asso:
            ...
            if np.any(is_new_lmk):
                ...
                z_new = z[z_new_inds]
                etaupd, Pupd, _, _ = self.update(eta, P, z_new)
